...
----------------------------   Filters List   --------------------------------
    ...
    1- Grayscale Conversion : just come the image white & Black & Gray colors.
    2- Black & white : just come the image white & Black colors.
    3- Invert Image : transforms the colors of an image by reversing each pixel's RGB values.
    4- Darking & Lighting : allows the user to make the image darker or lighter by 50% from the original image.
    5- Flip Image : involves mirroring its contents horizontally or vertically.
    6- Grayscale Conversion : just come the image white & Black & Gray colors.
    7- Black & white : just come the image white & Black colors.
    8- Invert Image : transforms the colors of an image by reversing each pixel's RGB values.
    9- Darking & Lighting : allows the user to make the image darker or lighter by 50% from the original image.
    10- Flip Image : involves mirroring its contents horizontally or vertically.

---------------------------------------------------------------------------------
created in : 2024\3\25.
...
Image detecting_image(string image_name)
{
    ...
    Image detect(image.width, image.height);

    for (int i = 2; i < image.width - 2; i++) {
        for (int j = 2; j < image.height - 2; j++) {
            for (int k = 0; k < image.channels; k++) {
                detect(i,j,k) = (  image(i-2,j-2,k)*0 + image(i-1,j-2,k)*0 + image(i,j-2,k)*0 +
    ...
    for (int i = 0; i < detect.width ; i++) {
        for (int j = 0; j < detect.height ; j++) {
            for (int k = 0; k < detect.channels; k++) {
                if (detect(i,j,k) > 128)
                {
                    detect(i,j,k) = 0;
                }
...
Image merge_image(string file_name,string file_name2)
{
    ...
    for (int i = 0; i < image1.width; i++)
    {
        for (int j = 0; j < image1.height; j++)
        {
            for (int k = 0; k < image1.channels; k++)
            {
                MergePixel = (image1(i, j, k) + image2(i, j, k)) / 2;
